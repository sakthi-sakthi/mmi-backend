...
@section('content')
    <div class="content-wrapper">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">{{ __('main.Editourteam') }}</li>
                        </ol>
                        </ol>
                    </div>
                </div>
        ...
        <div class="content">
            <div class="container-fluid">
                <form action="{{ route('admin.ourteams.update', $ourteam->id) }}" method="post" id="form">
                    @method('PUT')
                    @csrf
                    <div class="row">
                        <div class="col-md-9">
                            <div class="card">
                                <div class="card-body">
                                    <input type="hidden" id="category_id" name="category_id"
                                        value="{{ old('category_id') ?? $ourteam->getCategory->id }}">
                                    <input type="hidden" name="media_id" id="media_id"
                                        value="{{ old('media_id') ?? $ourteam->getMedia->id }}">
                                    <div class="form-group">
                                        <label for="title">{{ __('main.Title') }} <span class="text-danger">*</span> </label>
                                        <input type="text"
                                            class="form-control form-control-sm @error('title') is-invalid @enderror"
                                            id="title" name="title" value="{{ old('title') ?? $ourteam->title }}">
                                        @error('title') <small
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text"
                                            ...
                                            id="role" name="role" value="{{ old('role') ?? $ourteam->role }}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control form-control-sm" rows="3" id="content"
                                            name="content">{{ old('content') ?? $ourteam->content }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <img src="{{ $ourteam->getMedia->id == 1 ? '' : $ourteam->getMedia->getUrl() ?? '' }}"
                                    ...
                                    <div class="form-group">
                                        <select class="form-control form-control-sm" id="category" name="category">
                                            ...
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}" @if (old('category') ?? $ourteam->getCategory->id == $category->id) selected @endif>
                        ...
                        <div class="card" id="save-card">
                            <div class="card-body">
                                <a href="javascript:void(0);" class="btn btn-success btn-sm float-right"
                                    id="submitourteam">{{ __('main.Update') }}</a>

                        ...
                        <table id="table1" class="table table-bordered table-hover">
                            ...
                            <tbody>
                                @foreach ($articles as $article)
                                    <tr>
                                        <td>
                                            <img src="{{ $article->getMedia->id == 1 ? '' : $article->getMedia->getUrl('thumb') }}" alt="" style="width: 50px">
                                        </td>
                                        <td>{{ $article->title }}</td>
                                        <td>{{ $article->getCategory->title }}</td>
                                        <td>{{ $article->deleted_at->diffForHumans() }}</td>
                                        <td>
                                            <a href="{{ route('admin.testimonial.recover', $article->id) }}"
                                                title="{{ __('main.Recover') }}" class="btn btn-warning btn-xs"><i
                                                    class="fas fa-recycle"></i></a>
                                            <form id="delete_{{ $article->id }}"
                                                action="{{ route('admin.testimonial.destroy', $article->id) }}" method="post"
                                                class="d-inline">
                                                @method('DELETE')
                                                @csrf

    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h4 class="m-0 text-dark">{{ __('main.subchildsubmenu') }}</h4>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">{{ __('main.Home') }}</a>
                            </li>
                            <li class="breadcrumb-item active">{{ __('main.subchildsubmenu') }}</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        <a href="{{ route('admin.subchildsubmenu.create') }}"
                            class="btn btn-success btn-sm">{{ __('main.Add New') }}</a>
                        <a href="{{ route('admin.subchildsubmenu.trashed') }}" class="btn btn-warning btn-sm float-right"><i
                            ...
                            <tbody id="subcontent">
                                @foreach ($articles as $article)
                                    <tr class="row1" data-id="{{ $article->id }}" data-row-id={{ $article->Position }} >
                                        <td>{{ $article->childsubmenuname }}</td>
                                        <td>{{ $article->title }}</td>
                                        <td>{{ $article->link }}</td>
                                        ...
                                        <td>  
                                            <a href="{{ route('admin.subchildsubmenu.edit', $article->id) }}"
                                                title="{{ __('main.Edit') }}" class="btn btn-primary btn-xs"><i
                                                    class="fas fa-pencil-alt"></i></a>
                                            <a href="{{ route('admin.subchildsubmenu.delete', $article->id) }}"
...
@section('script')
    ...
    <script>
        ...
        $.get("{{ secure_url(route('admin.subchildsubmenu.switch')) }}", {
            id: id,
            status: status
        });
    });
...
function updatePostOrder(row){
    var Position = [];
     var token = $('meta[name="csrf-token"]').attr('content');
        ...
        $.post("{{ secure_url(route('admin.subchildsubmenu.updateorder')) }}", {
         Position: Position,
         _token: token
    })

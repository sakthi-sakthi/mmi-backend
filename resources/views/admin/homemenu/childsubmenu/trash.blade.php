                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item"><a
                                    href="{{ route('admin.childsubmenu.index') }}">{{ __('main.childsubmenu') }}</a></li>
                            ...
                            <tbody>
                                @foreach ($articles as $article)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.childsubmenu.recover', $article->id) }}"
                                                title="{{ __('main.Recover') }}" class="btn btn-warning btn-xs"><i
                                                    class="fas fa-recycle"></i></a>
                                            <form id="delete_{{ $article->id }}"
                                                action="{{ route('admin.childsubmenu.destroy', $article->id) }}" method="post"
                                                class="d-inline">
                                                @method('DELETE')
                                                @csrf

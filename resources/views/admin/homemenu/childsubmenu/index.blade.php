    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h4 class="m-0 text-dark">{{ __('main.childsubmenu') }}</h4>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">{{ __('main.Home') }}</a>
                            </li>
                            <li class="breadcrumb-item active">{{ __('main.childsubmenu') }}</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        <a href="{{ route('admin.childsubmenu.create') }}"
                            class="btn btn-success btn-sm">{{ __('main.Add New') }}</a>
                        <a href="{{ route('admin.childsubmenu.trashed') }}" class="btn btn-warning btn-sm float-right"><i
                            ...
                            <tbody id="subcontent">
                                @foreach ($articles as $article)
                                    <tr class="row1" data-id="{{ $article->id }}" data-row-id={{ $article->Position }} >
                                        ...
                                        <td>  
                                            <a href="{{ route('admin.childsubmenu.edit', $article->id) }}"
                                                title="{{ __('main.Edit') }}" class="btn btn-primary btn-xs"><i
                                                    class="fas fa-pencil-alt"></i></a>
                                            <a href="{{ route('admin.childsubmenu.delete', $article->id) }}"
...
function updatePostOrder(row){
    var Position = [];
     var token = $('meta[name="csrf-token"]').attr('content');
        ...
        $.post("{{ secure_url(route('admin.childsubmenu.updateorder')) }}", {
         Position: Position,
         _token: token
    })

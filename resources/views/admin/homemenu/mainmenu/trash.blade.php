                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item"><a
                                    href="{{ route('admin.mainmenu.index') }}">{{ __('main.mainmenu') }}</a></li>
                            ...
                            <tbody>
                                @foreach ($articles as $article)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.mainmenu.recover', $article->id) }}"
                                                title="{{ __('main.Recover') }}" class="btn btn-warning btn-xs"><i
                                                    class="fas fa-recycle"></i></a>
                                            <form id="delete_{{ $article->id }}"
                                                action="{{ route('admin.mainmenu.destroy', $article->id) }}" method="post"
                                                class="d-inline">
                                                @method('DELETE')
                                                @csrf

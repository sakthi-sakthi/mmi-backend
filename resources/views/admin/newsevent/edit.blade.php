...
@section('content')
    <div class="content-wrapper">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item"><a
                                    href="{{ route('admin.news_events.index') }}">{{ __('main.ourfeature') }}</a></li>
                                 
                                 <li class="breadcrumb-item active">{{ $newsevent->title }}</li>
                           </ol>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
        ...
        <div class="content">
            ...
            <div class="container-fluid">
                <form action="{{ route('admin.news_events.update',$newsevent->id) }}" method="post" id="form" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="row">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body"> 
                                    <input type="hidden" id="category_id" name="category_id"
                                        value="{{ old('category_id') ?? $newsevent->getCategory->id }}">
                                    <input type="hidden" name="media_id" id="media_id"
                                        value="{{ old('media_id') ?? $newsevent->getMedia->id }}">
                                    <div class="form-group">
                                        <label for="title">{{ __('main.Title') }}<span class="text-danger">*</span></label>
                                        <input type="text"
                                            class="form-control form-control-sm @error('title') is-invalid @enderror"
                                            id="title" name="title" value="{{ old('title') ?? $newsevent->title }}">
                                        @error('title') <small
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class=""><a href="{{ url('/UpcomingNews/'. $newsevent->file_id ) }}" download="download">{{ $newsevent->file_id }}</a></div>
                                        @error('file_id')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control form-control-sm" rows="3" id="content"
                                            name="content">{{ old('content') ?? $newsevent->content }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <img src="{{ $newsevent->getMedia->id == 1 ? '' : $newsevent->getMedia->getUrl() ?? '' }}"
                                    ...
                                    <div class="form-group">
                                        <select class="form-control form-control-sm" id="category" name="category">
                                            ...
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}" @if (old('category') ?? $newsevent->category_id == $category->id) selected @endif>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <input type="date" name="eventdate" id="eventdate" value="{{ old('eventdate') ?? $newsevent->eventdate ?? date('Y-m-d') }}" class="form-control form-control-sm @error('eventdate') is-invalid @enderror">
                                </div>
                            </div>
                        </div>
                        <div class="card" id="save-card">
                            <div class="card-body">
                                <a href="javascript:void(0);" class="btn btn-success btn-sm float-right"
                                    id="submitnewsletter">{{ __('main.Update') }}</a>
                                    <a href="{{ route('admin.news_events.index') }}" class="btn btn-danger btn-sm float-right mr-2">Cancel</a>
                            </div>
                        </div>
                    </div>
                </form>
